...
namespace EDMXMigrationTool
{
    public partial class frmCentralForm : Form
    {
        ...
        private void CreateDbContext(IDictionary<string, Entity> entities)
        {
            ...
            file.AppendLine(txtNamespace.Text);
            file.AppendLine("{");
            file.AppendLine("    public class AppDbContext : DbContext");

            file.Append("}");
            File.WriteAllText(Path.Combine(txtDestination.Text, "AppDbContext.cs"), file.ToString());
        }
